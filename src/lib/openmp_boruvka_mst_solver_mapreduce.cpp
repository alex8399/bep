...
void OpenMPBoruvkaMSTSolverMapReduce::calculateMST(const Graph &graph, Graph &mst, ExperimentSetup &experimentSetup)
{
    ...
    while (treesNum > 1)
    {
        ...
        {
            std::vector<int> &cheapestEdgeLocal = threadLocal[omp_get_thread_num()];
            ...
            if (pivot1 != pivot2)
            {
                if (cheapestEdgeLocal[pivot1] == NULL_EDGE || edge.weight < graph.edges[cheapestEdgeLocal[pivot1]].weight)
                {
                    cheapestEdgeLocal[pivot1] = ind;
                }
                if (cheapestEdgeLocal[pivot2] == NULL_EDGE || edge.weight < graph.edges[cheapestEdgeLocal[pivot2]].weight)
                {
        ...
        #pragma omp parallel for schedule(static)
        for (int pivot = 0; pivot < graph.verticesNum; ++pivot)
        {
            for (int thread = 0; thread < threadsNum; ++thread)
            {
                if (threadLocal[thread][pivot] != NULL_EDGE &&
                    (cheapestEdge[pivot] == NULL_EDGE
                    || graph.edges[threadLocal[thread][pivot]].weight < graph.edges[cheapestEdge[pivot]].weight))
                {
                    cheapestEdge[pivot] = threadLocal[thread][pivot];
                }
            }
        }
